...
from apps.admin_panel.dtos import AdminPanelDto
from apps.admin_panel.injectors import serv_admin_panel
from apps.admin_panel.middlewares.auth_utils import (
    authenticate_user,
    refresh_access_token,
)
from apps.admin_panel.middlewares.base_response_handler import BaseResponseHandler
from flask import jsonify, make_response, redirect, render_template, request, url_for
from flask_restx import Resource
from apps.admin_panel.constant_config import PREFIX
...
class AdminPanelIndexController(Resource):
    ...
    def get(self):
        ...
        html_content = render_template(
            ...
            app_title=serv_admin_panel.app_title,
...
class AdminPanelListModelController(Resource):
    ...
    def get(self):
        ...
        """
        lst_models = list(serv_admin_panel.models_for_admin_panel.keys())
        headers = {"Content-Type": "text/html"}
        html_content = render_template(
            "admin_panel/models_list.html",
            app_title=serv_admin_panel.app_title,
            models=lst_models,
        )
        return make_response(html_content, 200, headers)
...
class AdminPanelListRecordsController(Resource):
    ...
    def get(self, model_name):
        ...
        """
        lst_records, lst_columns = serv_admin_panel.list_records(model_name)
        headers = {"Content-Type": "text/html"}
        html_content = render_template(
            "admin_panel/list.html",
...
class AdminPanelEditRecordController(Resource):
    ...
    def get(self, model_name, id):
        ...
        """
        try:
            check, model_name, form = serv_admin_panel.edit_record(
    ...
    def post(self, model_name, id):
        ...
        try:
            ...
            if check:
                return redirect(
                    url_for(
                        "admin_panel.admin_panel_admin_panel_list_records_controller",
                        model_name=model_name,
                    )
                )
            else:
...
class AdminPanelAddRecordController(Resource):
    ...
    def get(self, model_name):
        ...
        """
        try:
            check, model_name, form = serv_admin_panel.add_record(request, model_name)
            headers = {"Content-Type": "text/html"}
            html_content = render_template(
                "admin_panel/form.html",
                model_name=model_name,
...
class AdminPanelDeleteRecordController(Resource):
    ...
    def post(self, model_name, id):
        ...
        """
        try:
            check, model_name, form = serv_admin_panel.delete_record(
                request, model_name, id
            )
            if check:
                return redirect(
